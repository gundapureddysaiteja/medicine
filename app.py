...
os.environ["GOOGLE_API_KEY"] = "AIzaSyDUSqbSN_LI-mGjd_oa9QipoUqYUSZGoKA"

# Load data for disease prediction
...
# Load data for medicine recommendation
...
def recommend_medicine(medicine):
...
def predict_disease():
    if request.method == 'POST':
        ...
        if invalid_symptoms:
            message = f"Invalid symptoms detected: {', '.join(invalid_symptoms)}. Please correct them."
            return render_template('symptoms.html', message=message)
...
def recommend():
    ...
    if request.method == 'POST':
        ...
        if selected_medicine_name:
            recommendations = recommend_medicine(selected_medicine_name)
    
    medicines_list = medicines['Drug_Name'].tolist()
    return render_template('medicine.html', medicines=medicines_list, recommendations=recommendations)
